...
class CapitalsDatasetGlyphnet(Dataset):
...
def data_loader(path: str, batch_size: int, n_glyphs_visible: int, glyphs_visible: list[int], use_mask: bool, default_font: str, seed: int, use_skeleton_mask: bool, masks_path: str):
    g = torch.Generator()
    g.manual_seed(seed)
    data = CapitalsDatasetGlyphnet(root_dir=path, n_glyphs_visible=n_glyphs_visible,glyphs_visible=glyphs_visible, use_mask=use_mask,default_font_path=default_font,use_skeleton_mask=use_skeleton_mask,masks_path=masks_path)
