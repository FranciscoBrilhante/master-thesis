...
class GeneratorPart1(nn.Module):
    def __init__(self):
        super(GeneratorPart1, self).__init__()

        self.generator_part1 = nn.Sequential(
        ...
        self.final=nn.Linear(576*16*16,128)

    def forward(self, x: Tensor):
        out = self.generator_part1(x)
        out = out.view(-1, 576*16*16)
        out=self.final(out)
        return out

class GeneratorPart2(nn.Module):
    def __init__(self):
        super(GeneratorPart2, self).__init__()

        self.initial=nn.Linear(128,576*16*16)
        self.generator_part2 = nn.Sequential(
...
class GlyphGan(nn.Module):
    def __init__(self, lr: float, epochs_decay_lr: int, is_train: bool, beta: float, device: str, l1_lambda: float, use_mask:bool):
        ...
        self.generator_part1 = GeneratorPart1()
        self.generator_part2=GeneratorPart2()
        ...
        if self.is_train:
            ...
            params = [self.generator_part1.parameters(), self.generator_part2.parameters()]
            self.optimizer_generator = Adam(itertools.chain(*params), lr=self.lr, betas=(self.beta, 0.999))
    ...
    def print_network(self):
        for net in [self.generator_3d, self.generator_part1, self.generator_part2, self.discriminator_local, self.discriminator_global]:
            num_params = 0
    ...
    def labels_to_tensor(self, labels, size) -> Tensor:
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        output = self.generator_part1.forward(input_indep.squeeze(2))
        output= self.generator_part2(output)
